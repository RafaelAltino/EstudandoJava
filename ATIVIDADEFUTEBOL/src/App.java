...
public class App {
    public static void main(String[] args) throws Exception {
        ...
        exibirJogadores(times ,Tipo.MEIA);
            
    }

    public static void exibirJogadores(List<Time> times ,Tipo funcao){

        times.stream()
            .flatMap(time -> 
                time.getJogadores().stream()
                    .filter( jogador -> jogador.getTipo().equals(funcao))
                    .peek( jogador -> System.out.println(jogador.getNome() + " -> " + time.getNome()))                                        
            )
